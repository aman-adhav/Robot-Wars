...
/**
 * Robot Summative : Main Bot
 * @author Aman Adhav
 * @version Date 2016-01-26
 */
public class AmanBot extends FighterRobot {
	/*
	 * To Mr Ching
	 * I would like to run this program in the game on Wednesday
	 * For my actual program please refer to the AmanBot2
	 */
	//Variables
	private int health=100;
	private int completemoves = 0;
	 ...
	 */
	public AmanBot(City arena, int a, int s,Direction direction, int id, int attack) {
	...
	private AmanBotOppData[] oppDataPrepare(OppData[] w) {
		...
		else
		{
			...
			{	
				getval[i].setStreet(w[i].getStreet());
				getval[i].setAvenue(w[i].getAvenue());
				getval[i].setHealth(w[i].getHealth());
	...
	private int huntY(int turnY) {
		...
		if (nowY > turnY){
			...
			while (turnY < nowY && 0 < this.maxmoves){
	...
	private int huntX(int turnX) {
		...
		if (nowX > turnX){
			...
			//Counts while the number of steps allowed is not breached
			while (turnX < nowX && 0 < this.maxmoves){
	...
	private int kill(AmanBotOppData[] w) {
		...
		for (int i =0; i<w.length; i++){
			...
			if(this.completemoves == 0){
				...
				this.stalkplayer[i].setcountmoves(w[i].getAvenue(), w[i].getStreet());
			}
		}
		...
		fig = easyKill(healthList,distanceList);
		//distanceList = this.sortDistance(w);
	 ...
	 */
	private int easyKill(OppData[] healthList, OppData[] distanceList) {
		...
		for (int q = 0; q<distanceList.length;q++){
			...
			for (int i = q; i<distanceList.length;i++){
				...
				if (this.getID() != i && w[i].getHealth()>0){
					...
					nextX = Math.abs(w[i].getAvenue()-this.getAvenue());
					nextY = Math.abs(w[i].getStreet()-this.getStreet());
					
					//Finds the smallest distance
					if(nextX+nextY<totalD && i>fig){
						totalD = nextX+nextY;
						fig = i;
					}
